...
public class HashTable<Key, Value> implements Map<Key, Value> {
    ...
    private Node<Key, Value>[] map = (Node<Key, Value>[]) new Node[tableSize];
    private float tableLoad = 0;
    private HashingMethod hashingMethod = pickHashMethod();
    ...
    public void add(Key key, Value value) {
        ...
        Node<Key, Value> newTableEntry = new Node<>(key, value,null);             //create the node to be added (value, key, nextReference)
        ...
        for(int i = 0; i < oldMap.length; i++){
            if(oldMap[i] != null){                                                                             //if position on oldTable have something, transfer it to new table
                ...
                while (rehash != null){                                                                       //rehash all nodes in a table entry (bucket)
                    ...
                    Node<Key,Value> newEntryPosition = new Node<>(rehash.hashKey, rehash.nodeValue,null);
    ...
    private void addNextNode(Node<Key,Value> entry, int mapPosition) {      //insert nodes on the new one to CHAIN them out
    ...
    public Value get(Key key) {
        ...
        Node<Key, Value> search = map[hashPosition];
        while (search != null){
    ...
    public Value delete(Key key) {
        int hashPosition = hashingMethod == HashingMethod.Weighted ?
                getHashValueW(key):getChrisUniversalHashing(key);       //Assign a number in the table
        Node<Key, Value> front = map[hashPosition];
    ...
    public Value replace(Key key, Value newValue) {
        int hashPosition = hashingMethod == HashingMethod.Weighted?
                getHashValueW(key):getChrisUniversalHashing(key);       //Assign a number in the table
        Node<Key, Value> replace = map[hashPosition];
        while (replace != null) {
            if (key.equals(replace.hashKey)) { // replace this node value
                Value oldValue =  replace.nodeValue;
